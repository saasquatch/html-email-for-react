...
import { MetaContext } from "./raw-components";

export { raw } from "./raw-components";
...
export function render<T>(
    ...
    replace: function (node: DomElement) {
      ...
      if (node.name && hasComponents) {
        ...
        if (Component) {
          ...
          const children =
            node.children && domToReact(node.children, parserOptions);
  ...
  const base = (
    <MetaContext.Provider value={[DataContext]}>
      <DataContext.Provider value={data}>{reactElement}</DataContext.Provider>
    </MetaContext.Provider>
  );
  const pimped = renderToStaticMarkup(base);
